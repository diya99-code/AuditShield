...
class ActionHandler:
    ...
    def _open_document(
        ...
        if doc_name in state.opened_documents:
            state.loop_detected = True
        else:
            state.opened_documents.append(doc_name)
        ...
        # Map to required_checks schema
        check_map = {
            "invoice": "opened_invoice",
            "purchase_order": "opened_purchase_order",
            "goods_receipt": "opened_goods_receipt",
            "invoice_history": "opened_invoice_history",
            "ap_policy": "opened_ap_policy",
        }
        if doc_name in check_map:
            state.checks_completed[check_map[doc_name]] = True
        
        # Hard task special check
        if doc_name == "goods_receipt":
             state.checks_completed["checked_partial_receipt"] = True
    ...
    def _compare_fields(
        ...
        if not matched:
            if "qty" in field_a.lower() or "qty" in field_b.lower():
                if "quantity_mismatch" not in state.identified_issues:
                    state.identified_issues.append("quantity_mismatch")

        return ActionResult(
            success=True,
            message=result_str,
            completed_comparison=True,
            identified_core_issue=not matched,
        )
    ...
    def _check_policy(
        ...
        state.checks_completed["applied_policy"] = True
        # Hard task issue detection
        if fixture.task_id == "hard_duplicate_partial":
            if "policy_condition" not in state.identified_issues:
                state.identified_issues.append("policy_condition")
    ...
    def _search_history(
        ...
        state.checks_completed["checked_duplicate_risk"] = True
        ...
        if duplicate_detected:
            if "duplicate_risk" not in state.identified_issues:
                state.identified_issues.append("duplicate_risk")
    ...
    def _terminal(
        ...
        # Special check for hard case partial receipt identification
        if fixture.task_id == "hard_duplicate_partial" and "goods_receipt" in state.opened_documents:
            if "partial_receipt" not in state.identified_issues:
                state.identified_issues.append("partial_receipt")
